...
echo "[deploy-cloud-run] probing health"
for attempt in 1 2 3 4 5 6; do
  if curl -fsS "${URL}/health" >/dev/null 2>&1; then
    echo "[deploy-cloud-run] health probe passed"
    exit 0
  fi
  echo "[deploy-cloud-run] health probe attempt ${attempt}/6 failed"
  sleep 5
done

echo "[deploy-cloud-run] health probe failed after deploy" >&2
exit 1
